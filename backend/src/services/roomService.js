...
function joinRoom(roomId, guestPlayerId) {
  ...
  if (
    guestPlayerId === room.hostPlayerId
  ){

    throw new Error(
      'Host and guest cannot be the same player'
    );
...
function setPlayerReady(roomId, playerId, ready) {
  ...
  if (
    ...
    throw new Error(
      'Players can only ready up while the room is in LOBBY'
    );

  }

  room.playersReady[playerId] =
  ready === true;
...
function submitQuestionSet(roomId, playerId, questionIds) {
  ...
  if (
    ...
    throw new Error(
      'Question decks can only be submitted while the room is in LOBBY'
    );
  ...
  if(
    ...
    &&
    room.submittedQuestionSets[playerId].length > 0
  ){

    throw new Error(
      "Player already submitted deck"
    );
...
function hasBothQuestionSets(roomId) {
  ...
  const hostDeck =
  room.submittedQuestionSets[room.hostPlayerId] || [];

  const guestDeck =
  room.submittedQuestionSets[room.guestPlayerId] || [];
...
function canStartGame(roomId) {
  ...
  const hostReady =
  room.playersReady[room.hostPlayerId] === true;

  const guestReady =
  room.playersReady[room.guestPlayerId] === true;
