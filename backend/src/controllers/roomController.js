...
function setReady(req, res, next) {
  ...
  try {
    ...
    if (
      roomService.canStartGame(roomId)
    ){
...
async function submitQuestionSet(req, res, next) {
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
        'playerId and questionIds required'
      });
    ...
    // ELASTIC VALIDATION (critical)
    await questionService.validateQuestionsExist(
      validatedQuestions
    );
    ...
    roomService.submitQuestionSet(
      ...
      validatedQuestions   // FIXED (no .map)
